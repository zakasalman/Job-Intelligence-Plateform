...
with tab1:
    ...
    col1.metric("Total Jobs Analyzed", "500K+")
    ...
    with chart_col1:
        ...
        fig_skills = px.bar(get_top_skills(df), x='Count', y='Skill', orientation='h', color='Count', color_continuous_scale='Blues', text_auto='.2s')
        ...
        fig_levels = px.pie(level_df, names='Experience Level', values='Count', hole=0.4, color_discrete_sequence=px.colors.sequential.Teal)
    ...
    with chart_col2:
        ...
        fig_loc = px.bar(loc_df, x='Location', y='Count', color='Count', color_continuous_scale='Viridis', text_auto='.2s')
        ...
        fig_type = px.pie(type_df, names='Job Type', values='Count', color_discrete_sequence=px.colors.sequential.Sunset)
...
with tab2:
    ...
    if st.button("Calculate ATS Match Score") and job_desc and resume_text:
        ...
        # Adjusted for realistic industry ATS thresholds
        if match_percentage >= 40:
            st.success(f"🔥 Excellent Match! Your ATS Score is **{match_percentage:.1f}%**")
        elif match_percentage >= 20:
            st.warning(f"⚠️ Moderate Match. Your ATS Score is **{match_percentage:.1f}%**. Consider tailoring your keywords.")
        else:
            st.error(f"❌ Low Match. Your ATS Score is **{match_percentage:.1f}%**. You are missing core requirements.")
